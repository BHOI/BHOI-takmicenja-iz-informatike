...
int main() {
    ...
    set<vector<int>> vidjena_stanja;  // Necemo isto stanje vise puta razmotriti, ovo znacajno ubrzava program
    while(q_stanja.front() != cilj_stanje) {
        ...
        if(vidjena_stanja.count(tr_stanje)) {
            continue;  // Ako je ovo stanje vec vidjeno, preskacemo ga
        }
        vidjena_stanja.insert(tr_stanje);  // Oznacavamo stanje kao vidjeno
