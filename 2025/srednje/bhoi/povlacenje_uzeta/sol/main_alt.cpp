/* Preuzeto sa Balticke Olimpijade iz Informatike 2015 - Tug of War
...
int main() {
  ...
  back_pack[0] = true;
  for (size_t i = 1; i < BACK_PACK_SIZE*1u; i++) {
    if (item[i] == 0u)
      continue;
    // cout<<"it"<<item[i]<<endl;
	for(int itsiz=i;item[i]!=0;itsiz=min(itsiz*2,int(item[i]*i))){
    for (int j = BACK_PACK_SIZE / 2; j >= 0; --j) {
      if (back_pack[j]) {
          back_pack[j +  itsiz] = true;
      }
