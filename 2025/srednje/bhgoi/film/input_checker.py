...
NUM_TESTCASES = 60       # Number of testcase files to generate
...
def check_test_case(case_num):
    # Read the input file
    input_file = f"input/input{case_num}.txt"
    ...
    if len(names) != len(set(names)):
        print(f"Test case {case_num} has duplicate names.")
        return False
    
    # Check for unique ratings
    if len(ratings) != len(set(ratings)):
        print(f"Test case {case_num} has duplicate ratings.")
        # Determine which rating is duplicated
        rating_counts = {}
        ...
        for rating, count in rating_counts.items():
            if count > 1:
                print(f"Rating {rating} is duplicated {count} times.")
                break
        return False
